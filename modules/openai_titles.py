...
class OpenAITitles:
    def __init__(self, openai_api_key, settings_file="settings.yaml") -> None:
        ...
        current_app.logger.info("Init with settings: %s", self.settings)
    ...
    def __load_settings(self, settings_file):
        try:
            ...
            current_app.logger.error("Error loading settings file: %s", e)
            return None
    ...
    def generate_title_from_text(self, text):
        ...
        if setting_prompt:
            ...
            current_app.logger.info("Raw LLM Output: %s", raw_content)
